USE [master]
GO
/****** Object:  StoredProcedure [dbo].[sp_firstcheck]    Script Date: 12-11-2018 00:47:02 ******/
SET ANSI_NULLS ON
GO
...
Create PROCEDURE [dbo].[sp_firstcheck]
    ...
    @OutputTableNamesqlcheckCache NVARCHAR(256) = NULL ,
    ...
    @LogMessageFinding VARCHAR(200) = 'Logged from sp_sqlcheckFirst',
...
DECLARE @StringToExecute NVARCHAR(MAX),
	...
	@whoisrunning NVARCHAR(MAX) = N'EXEC dbo.sp_whoisrunning @ShowSleepingSPIDs = ' + CONVERT(NVARCHAR(1), @ShowSleepingSPIDs) + N';',
    @sqlcheckCacheMinutesBack INT,
    @sqlcheckCacheSortOrder VARCHAR(50),
...
BEGIN
    ...
    BEGIN
		IF OBJECT_ID('master.dbo.sp_whoisrunning') IS NULL AND OBJECT_ID('dbo.sp_whoisrunning') IS NULL
		BEGIN
			PRINT N'sp_sp_whoisrunning is not installed in the current database_files.';
		END;
		ELSE
		BEGIN
			EXEC (@whoisrunning);
    ...
    IF OBJECT_ID('tempdb..#firstcheckresults') IS NOT NULL
        DROP TABLE #firstcheckresults;
    CREATE TABLE #firstcheckresults
        (
    ...
    IF @Seconds > 0
    INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, HowToStopIt, QueryPlan, StartTime, LoginName, NTUserName, ProgramName, HostName, DatabaseID, DatabaseName, OpenTransactionCount)
    SELECT 1 AS CheckID,
        1 AS Priority,
    ...
    IF @Seconds > 0 AND CAST(SERVERPROPERTY('edition') AS VARCHAR(100)) <> 'SQL Azure'
    BEGIN
        SET @StringToExecute = 'UPDATE #firstcheckresults SET Details = Details + '' Over the last 60 days, the full backup usually takes '' + CAST((SELECT AVG(DATEDIFF(mi, bs.backup_start_date, bs.backup_finish_date)) FROM msdb.dbo.backupset bs WHERE abr.DatabaseName = bs.database_name AND bs.type = ''D'' AND bs.backup_start_date > DATEADD(dd, -60, SYSDATETIMEOFFSET()) AND bs.backup_finish_date IS NOT NULL) AS NVARCHAR(100)) + '' minutes.'' FROM #firstcheckresults abr WHERE abr.CheckID = 1 AND EXISTS (SELECT * FROM msdb.dbo.backupset bs WHERE bs.type = ''D'' AND bs.backup_start_date > DATEADD(dd, -60, SYSDATETIMEOFFSET()) AND bs.backup_finish_date IS NOT NULL AND abr.DatabaseName = bs.database_name AND DATEDIFF(mi, bs.backup_start_date, bs.backup_finish_date) > 1)';
        EXEC(@StringToExecute);
    END;
    ...
    SELECT 4 AS CheckID,
        ...
        'Unfortunately, you can''t stop this, but you can prevent it next time. ' AS HowToStopIt,
        pl.query_plan AS QueryPlan,
        r.start_time AS StartTime,
        s.login_name AS LoginName,
        s.nt_user_name AS NTUserName,
    ...
    BEGIN
        SET @StringToExecute = N'INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, HowToStopIt, QueryPlan, QueryText, StartTime, LoginName, NTUserName, ProgramName, HostName, DatabaseID, DatabaseName, OpenTransactionCount)
            SELECT 5 AS CheckID,
                1 AS Priority,
    ...
    BEGIN
        INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding,  Details, HowToStopIt)
    ...
    IF @Seconds > 0
    INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding,  Details, HowToStopIt, StartTime, LoginName, NTUserName, ProgramName, HostName, DatabaseID, DatabaseName, QueryText, OpenTransactionCount)
    SELECT 8 AS CheckID,
        50 AS Priority,
        'Query Problems' AS FindingGroup,
    ...
    IF @Seconds > 0 
     ...
     BEGIN
        SET @StringToExecute = N'INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, HowToStopIt, StartTime, LoginName, NTUserName, ProgramName, HostName, DatabaseID, DatabaseName, QueryText, OpenTransactionCount)
    ...
    IF @Seconds > 0
    INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding,Details, HowToStopIt, StartTime, LoginName, NTUserName, ProgramName, HostName, DatabaseID, DatabaseName, QueryText)
    SELECT 9 AS CheckID,
        1 AS Priority,
        'Query Problems' AS FindingGroup,
    ...
    SELECT 34 AS CheckID,
        ...
        'Run sp_sqlcheckCache @SortOrder = ''memory grant'' to find queries with huge memory grants and tune them.' AS HowToStopIt
    ...
    INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, DetailsInt)
    ...
    SELECT 40 AS CheckID,
		...
		(SELECT COUNT(*) FROM sys.dm_exec_query_memory_grants WHERE queue_id IS NULL) AS DetailsInt	FROM sys.dm_exec_query_memory_grants AS Grants;
    ...
    IF @Seconds < 30
        ...
        /* If we're waiting less than 30 seconds, run this check now rather than wait til the end.
           ...
           end of our sp_sqlcheckFirst session. */
        INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, DetailsInt)
        SELECT 24, 50, 'Server Performance', 'High CPU Utilization', CAST(100 - SystemIdle AS NVARCHAR(20)) + N'%.', 100 - SystemIdle
            FROM (
                SELECT record,
		...
		INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, DetailsInt, HowToStopIt)
		SELECT TOP 1 
				23, 
				250, 
				'Server Info', 
		...
		INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, DetailsInt)
	    SELECT 28,	50,	'Server Performance', 'High CPU Utilization - Not SQL', CONVERT(NVARCHAR(100),100 - (y.SQLUsage + y.SystemIdle)) + N'% - Other Processes (not SQL Server) are using this much CPU. This may impact on the performance of your SQL Server instance', 100 - (y.SQLUsage + y.SystemIdle)
            FROM (
                SELECT record,
                    record.value('(./Record/SchedulerMonitorEvent/SystemHealth/SystemIdle)[1]', 'int') AS SystemIdle
    ...
    IF DATEDIFF(ss, @FinishSampleTime, SYSDATETIME()) > 10 AND @CheckProcedureCache = 1
        BEGIN
            ...
            INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details)
    ...
    ELSE IF @CheckProcedureCache = 1
		...
		*/
        ...
        INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, HowToStopIt, QueryPlan, QueryText, QueryStatsNowID, QueryStatsFirstID, PlanHandle)
        SELECT 17, 210, 'Query Stats', 'Most Resource-Intensive Queries', 
            ...
            UPDATE #firstcheckresults
                SET DatabaseID = CAST(attr.value AS INT),
                DatabaseName = DB_NAME(CAST(attr.value AS INT))
            FROM #firstcheckresults
                CROSS APPLY sys.dm_exec_plan_attributes(#firstcheckresults.PlanHandle) AS attr
    ...
    INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, HowToStopIt, DetailsInt)
	...
	IF EXISTS (SELECT * FROM #firstcheckresults WHERE Finding LIKE 'PAGEIOLATCH%')
	BEGIN
		INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, HowToStopIt, DatabaseID, DatabaseName)
	...
	IF EXISTS (SELECT * FROM #firstcheckresults WHERE Finding LIKE 'WRITELOG%')
	BEGIN
		INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, HowToStopIt, DatabaseID, DatabaseName)
    ...
    SELECT 15 AS CheckID,
        ...
        'To find the queries that are compiling, start with:' + @LineFeed
            + 'sp_sqlcheckCache @SortOrder = ''recent compilations''' + @LineFeed
    ...
    SELECT 32 AS CheckID,
        ...
        'Check out sp_sqlcheckCache to get more information about recent queries, or try sp_sqlcheckWho to see currently running queries.' AS HowToStopIt
    FROM #PerfmonStats ps
    ...
    IF @Seconds >= 30
        ...
        INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, DetailsInt)
        SELECT 24, 50, 'Server Performance', 'High CPU Utilization', CAST(100 - SystemIdle AS NVARCHAR(20)) + N'%. Ring buffer details: ' + CAST(record AS NVARCHAR(4000)), 100 - SystemIdle
            FROM (
                SELECT record,
                    record.value('(./Record/SchedulerMonitorEvent/SystemHealth/SystemIdle)[1]', 'int') AS SystemIdle
                FROM (
        ...
        INSERT INTO #firstcheckresults (CheckID, Priority, FindingsGroup, Finding, Details, DetailsInt)
        SELECT 23, 250, 'Server Info', 'CPU Utilization', CAST(100 - SystemIdle AS NVARCHAR(20)) + N'%. Ring buffer details: ' + CAST(record AS NVARCHAR(4000)), 100 - SystemIdle
            FROM (
                SELECT record,
                    record.value('(./Record/SchedulerMonitorEvent/SystemHealth/SystemIdle)[1]', 'int') AS SystemIdle
                FROM (
    ...
    RAISERROR('Analysis finished, outputting results',10,1) WITH NOWAIT;
        ...
        IF EXISTS (SELECT * FROM #firstcheckresults WHERE CheckID = 30)
            BEGIN
            SELECT TOP 1 @sqlcheckCacheSortOrder = CASE
        ...
        IF @sqlcheckCacheSortOrder IS NULL AND EXISTS (SELECT * FROM #firstcheckresults WHERE CheckID = 34)
            SET @sqlcheckCacheSortOrder = 'memory grant';

        /* Next, Compilations/Sec High - CheckID 15 and 16 */
        IF @sqlcheckCacheSortOrder IS NULL AND EXISTS (SELECT * FROM #firstcheckresults WHERE CheckID IN (15,16))
            SET @sqlcheckCacheSortOrder = 'compilations';

        /* Still not set? Use the top wait type. */
        IF @sqlcheckCacheSortOrder IS NULL AND EXISTS (SELECT * FROM #firstcheckresults WHERE CheckID = 6)
            BEGIN
            SELECT TOP 1 @sqlcheckCacheSortOrder = CASE
    ...
    BEGIN
        ...
        SET @StringToExecute = N' IF EXISTS(SELECT * FROM '
            ...
            + @OutputSchemaName + '.'
            + @OutputTableName
            + ' (ServerName, CheckDate, CheckID, Priority, FindingsGroup, Finding,  Details, HowToStopIt, QueryPlan, QueryText, StartTime, LoginName, NTUserName, OriginalLoginName, ProgramName, HostName, DatabaseID, DatabaseName, OpenTransactionCount, DetailsInt) SELECT '
            + ' @SrvName, @CheckDate, CheckID, Priority, FindingsGroup, Finding, Details, HowToStopIt, QueryPlan, QueryText, StartTime, LoginName, NTUserName, OriginalLoginName, ProgramName, HostName, DatabaseID, DatabaseName, OpenTransactionCount, DetailsInt FROM #firstcheckresults ORDER BY Priority , FindingsGroup , Finding , Details';
    ...
    ELSE
        ...
        BEGIN
			...
			ELSE
			BEGIN
			SELECT 'Plan Cache' AS [Pattern], 'Plan cache not analyzed' AS [Finding], 'Use @CheckProcedureCache = 1 or run sp_sqlcheckCache for more analysis' AS [More Info], CONVERT(XML, @StockDetailsHeader + '' + @StockDetailsFooter) AS [Details];
			END;
        END;
...
IF @SinceStartup = 0 AND @Seconds > 0 AND @ExpertMode = 1 
    BEGIN
		IF OBJECT_ID('master.dbo.sp_whoisrunning') IS NULL AND OBJECT_ID('dbo.sp_sqlcheckWho') IS NULL
		BEGIN
			PRINT N'sp_whoisrunning is not installed in the current database_files';
		END;
		ELSE
		BEGIN
			EXEC (@whoisrunning);
